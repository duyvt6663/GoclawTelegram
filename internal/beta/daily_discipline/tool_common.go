...
func (f *DailyDisciplineFeature) resolveToolConfig(ctx context.Context, key string) (*SurveyConfig, error) {
	...
	enabled := make([]SurveyConfig, 0, len(configs))
	...
	return nil, fmt.Errorf("config key is required when multiple daily discipline configs are available")
}

func resolveToolIdentity(ctx context.Context, args map[string]any) userIdentity {
	...
	if rawUserID == "" {
		if senderID := strings.TrimSpace(store.SenderIDFromContext(ctx)); senderID != "" {
			identity := parseUserIdentity(senderID)
			if rawLabel == "" {
	...
	return userIdentity{ID: rawUserID, Label: rawLabel}
}
